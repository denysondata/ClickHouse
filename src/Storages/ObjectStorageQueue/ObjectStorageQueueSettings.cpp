...
#define OBJECT_STORAGE_QUEUE_RELATED_SETTINGS(DECLARE, ALIAS) \
    ...
    DECLARE(UInt32, persistent_processing_node_owner_absence_grace_seconds, 10 * 60, "Minimum time a persistent processing claim must be unrefreshed while its active registry owner is absent before it can be cleaned up", 0) \
