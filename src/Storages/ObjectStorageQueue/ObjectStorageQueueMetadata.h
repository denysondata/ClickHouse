...
public:
    ...
    size_t getPersistentProcessingNodeOwnerAbsenceGraceSeconds() const
    {
        return persistent_processing_node_owner_absence_grace_seconds.load();
    }
    size_t getPersistentProcessingNodeRefreshIntervalSeconds() const
    {
        ...
        const size_t grace_seconds = persistent_processing_node_owner_absence_grace_seconds.load();
        const size_t lease_seconds = ttl_seconds && grace_seconds
            ? std::min(ttl_seconds, grace_seconds)
            : std::max(ttl_seconds, grace_seconds);
...
private:
    ...
    std::atomic<size_t> persistent_processing_node_owner_absence_grace_seconds;
