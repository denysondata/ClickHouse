...
ObjectStorageQueueMetadata::ObjectStorageQueueMetadata(
    ...
    size_t persistent_processing_node_owner_absence_grace_seconds_,
    ...
    , cleanup_processing_files(
        ...
        && (persistent_processing_nodes_ttl_seconds_ || persistent_processing_node_owner_absence_grace_seconds_))
    ...
    , persistent_processing_node_owner_absence_grace_seconds(persistent_processing_node_owner_absence_grace_seconds_)
...
void ObjectStorageQueueMetadata::cleanupThreadFuncImpl()
{
    ...
    /// Both settings are changeable at runtime and zero disables the corresponding
    /// cleanup condition.
    if (cleanup_processing_files
        && (persistent_processing_node_ttl_seconds || persistent_processing_node_owner_absence_grace_seconds))
...
void ObjectStorageQueueMetadata::updateSettings(const SettingsChanges & changes)
{
    for (const auto & change : changes)
    {
        ...
        if (change.name == "persistent_processing_node_owner_absence_grace_seconds")
            persistent_processing_node_owner_absence_grace_seconds = change.value.safeGet<UInt64>();
    }
}
...
void ObjectStorageQueueMetadata::cleanupPersistentProcessingNodes()
{
    ...
    auto get_paths = [&]
    {
        ...
        for (size_t i = 0; i < response.size(); ++i)
        {
            ...
            const size_t owner_absence_grace_seconds = persistent_processing_node_owner_absence_grace_seconds.load();
            LOG_TEST(
                log,
                "Node: {}, mtime: {}, ttl sec: {}, owner absence grace sec: {}, current time: {}",
                get_batch[i],
                response[i].stat.mtime,
                ttl_seconds,
                owner_absence_grace_seconds,
                current_time);
            ...
            if (!owner_absence_grace_seconds || node_mtime + owner_absence_grace_seconds >= current_time)
                continue;
